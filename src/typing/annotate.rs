...
fn annotate_pipes(pipes: &[Term], tenv: &mut TypeEnv) -> TyTerm {
    ...
    for t in it {
        ...
        let t = match t {
            ...
            Term::ViewFn(ref v_f) => TyTerm::TyFnApp(box annotate_view_fn(&v_f, &prev_arg, tenv)),
            _ => unimplemented!(),
        };
        term0 = t.clone();
    }
...
fn annotate_decl(decl: &Decl, tenv: &mut TypeEnv) -> TyDecl {
    ...
    let ret = match decl {
        NodeDecl(ref decl) => {
            ...
            for a in assigns {
                tenv.import_node_assign(&module, a);
            }
...
fn annotate_tensor_ty_sig(sig: &TensorTy, tenv: &mut TypeEnv, _span: &ByteSpan) -> Type {
    use self::TensorTy::*;
    let module = tenv.module();
    match sig {
